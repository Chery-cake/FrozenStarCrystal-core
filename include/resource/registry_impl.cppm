...
template <typename... Args>
  requires(!std::is_same_v<Policy, WeakPtrPolicy<Tag, Resource>> &&
           std::is_constructible_v<Resource, const Tag &, Args...> &&
...
template <typename... Args>
  requires(!std::is_same_v<Policy, WeakPtrPolicy<Tag, Resource>> &&
           ...
           std::is_constructible_v<Resource, Args...>)
bool Registry<Tag, Resource, Policy>::emplace(const Tag *tag, Args &&...args) {
  return add(tag, Policy::make_resource(std::forward<Args>(args)...));
}
...
typename Policy::StoredType
Registry<Tag, Resource, Policy>::getStored(const Tag *tag) const {
  std::unique_lock lock(mutex_);
  auto it = resources_.find(tag);
  if (it == resources_.end()) {
    return {};
