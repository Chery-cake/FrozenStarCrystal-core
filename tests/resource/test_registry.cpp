...
void test_add_unique() {
  ...
  static tags T5{.x = 6, .y = 4.6};
  const tags T6{.x = 8, .y = 8.2};
  tags T7{.x = 1, .y = 5.3};
  ...
  unique.add(&T5, std::make_unique<resources>(T5));
  unique.add(&T6, std::make_unique<resources>(T6));
  unique.add(&T7, std::make_unique<resources>(T7));

  assert(unique.size() == 8);
  ...
  assert(unique.get(&T4) != unique.get(&T5));
  assert(unique.get(&T5) != unique.get(&T6));
  assert(unique.get(&T6) != unique.get(&T7));
  ...
  assert(unique.getEntry(&T4) != unique.getEntry(&T5));
  assert(unique.getEntry(&T5) != unique.getEntry(&T6));
  assert(unique.getEntry(&T6) != unique.getEntry(&T7));
...
void test_add_shared() {
  ...
  shared.add(&T5, std::make_shared<resources>(T5));
  shared.add(&T6, std::make_shared<resources>(T6));
  shared.add(&T7, std::make_shared<resources>(T7));
  ...
  assert(shared.get(&T4) != shared.get(&T5));
  assert(shared.get(&T5) != shared.get(&T6));
  assert(shared.get(&T6) != shared.get(&T7));

  auto s0 = shared.getStored(&tagsCreated::T0);
  auto s1 = shared.getStored(&tagsCreated::T1);
  auto s2 = shared.getStored(&tagsCreated::T2);
  auto s3 = shared.getStored(&T3);
  auto s4 = shared.getStored(&T4);

  auto s5 = shared.getStored(&T5);
  auto s6 = shared.getStored(&T6);
  auto s7 = shared.getStored(&T7);

  assert(shared.getStored(&tagsCreated::T0) == s0);
  assert(shared.getStored(&tagsCreated::T1) == s1);
  assert(shared.getStored(&tagsCreated::T2) == s2);
  assert(shared.getStored(&T3) == s3);
  assert(shared.getStored(&T4) == s4);

  assert(shared.getStored(&T5) == s5);
  assert(shared.getStored(&T6) == s6);
  assert(shared.getStored(&T7) == s7);

  // 1 registry, 1 extra var, 1 var to check count
  assert(shared.getStored(&tagsCreated::T0).use_count() == 3);
  assert(shared.getStored(&tagsCreated::T1).use_count() == 3);
  assert(shared.getStored(&tagsCreated::T2).use_count() == 3);
  assert(shared.getStored(&T3).use_count() == 3);
  assert(shared.getStored(&T4).use_count() == 3);

  assert(shared.getStored(&T5).use_count() == 3);
  assert(shared.getStored(&T6).use_count() == 3);
  assert(shared.getStored(&T7).use_count() == 3);
  ...
  // 1 registry, 1 var to check count
  assert(shared.getStored(&tagsCreated::T0).use_count() == 2);
  assert(shared.getStored(&tagsCreated::T1).use_count() == 2);
  assert(shared.getStored(&tagsCreated::T2).use_count() == 2);
  assert(shared.getStored(&T3).use_count() == 2);
  assert(shared.getStored(&T4).use_count() == 2);

  assert(shared.getStored(&T5).use_count() == 2);
  assert(shared.getStored(&T6).use_count() == 2);
  assert(shared.getStored(&T7).use_count() == 2);
  ...
  assert(shared.getEntry(&T4) != shared.getEntry(&T5));
  assert(shared.getEntry(&T5) != shared.getEntry(&T6));
  assert(shared.getEntry(&T6) != shared.getEntry(&T7));
...
void test_add_weak() {
  ...
  auto sp5 = std::make_shared<resources>(T5);
  auto sp6 = std::make_shared<resources>(T6);
  auto sp7 = std::make_shared<resources>(T7);
  ...
  assert(weak.get(&T4) != weak.get(&T5));
  assert(weak.get(&T5) != weak.get(&T6));
  assert(weak.get(&T6) != weak.get(&T7));

  auto w0 = weak.getStored(&tagsCreated::T0);
  auto w1 = weak.getStored(&tagsCreated::T1);
  auto w2 = weak.getStored(&tagsCreated::T2);
  auto w3 = weak.getStored(&T3);
  auto w4 = weak.getStored(&T4);

  auto w5 = weak.getStored(&T5);
  auto w6 = weak.getStored(&T6);
  auto w7 = weak.getStored(&T7);

  assert(weak.getStored(&tagsCreated::T0).lock() == sp0);
  assert(weak.getStored(&tagsCreated::T1).lock() == sp1);
  assert(weak.getStored(&tagsCreated::T2).lock() == sp2);
  assert(weak.getStored(&T3).lock() == sp3);
  assert(weak.getStored(&T4).lock() == sp4);

  assert(weak.getStored(&T5).lock() == sp5);
  assert(weak.getStored(&T6).lock() == sp6);
  assert(weak.getStored(&T7).lock() == sp7);

  assert(weak.getStored(&tagsCreated::T0).lock() == w0.lock());
  assert(weak.getStored(&tagsCreated::T1).lock() == w1.lock());
  assert(weak.getStored(&tagsCreated::T2).lock() == w2.lock());
  assert(weak.getStored(&T3).lock() == w3.lock());
  assert(weak.getStored(&T4).lock() == w4.lock());

  assert(weak.getStored(&T5).lock() == w5.lock());
  assert(weak.getStored(&T6).lock() == w6.lock());
  assert(weak.getStored(&T7).lock() == w7.lock());

  // 1 registry
  assert(weak.getStored(&tagsCreated::T0).use_count() == 1);
  assert(weak.getStored(&tagsCreated::T1).use_count() == 1);
  assert(weak.getStored(&tagsCreated::T2).use_count() == 1);
  assert(weak.getStored(&T3).use_count() == 1);
  assert(weak.getStored(&T4).use_count() == 1);

  assert(weak.getStored(&T5).use_count() == 1);
  assert(weak.getStored(&T6).use_count() == 1);
  assert(weak.getStored(&T7).use_count() == 1);
  ...
  assert(weak.getEntry(&T4) != weak.getEntry(&T5));
  assert(weak.getEntry(&T5) != weak.getEntry(&T6));
  assert(weak.getEntry(&T6) != weak.getEntry(&T7));
